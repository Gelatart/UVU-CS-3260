...
/// CS3260 Section 001
/// Project: Lab_12
/// Date: 4/24/2020 9:28:45 PM
/// <summary>
/// Program with all of the data to query through, and the query tests that display results to console
/// </summary>
///  
...
namespace Lab12
{
    ...
    {
        /// <summary>
        /// Contains the product list to query through
        /// </summary>
        ...
        /// <summary>
        /// Contains the other data to query through and the queries and results
        /// </summary>
        ...
        static void Main(string[] args)
        {
            ...
            IEnumerable<int> query0 = odds.Where(n => n % 3 == 0);
            //foreach (Customer cust in firstReport) Console.WriteLine(cust.First + "\n" + cust.Last + "\n" + cust.Price + "\n");
            Console.WriteLine("Query0:");
            foreach (int value in query0) Console.WriteLine(value);
            var query1 =
                from even in evens
                from odd in odds
                where even > 2 * odd
                select new { even, odd };
            Console.WriteLine("\nQuery1:");
            foreach (var value in query1) Console.WriteLine(value);
            var query2 =
                from dat in data
                where dat.ID > 100 && dat.ID < 500 && dat.Name.Contains("y") || dat.Job.EndsWith("e") || dat.Job.Contains("e") || dat.Job == "Student" || dat.Job == "Programmer"
                select new { dat };
            Console.WriteLine("\nQuery2:");
            foreach (var value in query2) Console.WriteLine(value);
            var query3 =
            ...
            Console.WriteLine("\nQuery3:");
            foreach (var value in query3) Console.WriteLine(value.ProductName + " " + value.UnitsInStock);
            var query4 =
                from product in GetProductList()
                let p = product.ProductName.ToLower()
                join odd in odds on product.ProductID equals odd
                where product.Category == "Beverages" && product.UnitsInStock > 20
                orderby p ascending, product.ProductID descending
                select new { product.ProductID, p };
            Console.WriteLine("\nQuery4:");
            foreach (var value in query4) Console.WriteLine(value.ProductID + " " + value.p);
            Console.ReadKey();
        }
    }
    /// <summary>
